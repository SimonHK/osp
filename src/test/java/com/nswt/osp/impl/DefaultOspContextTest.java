...
import com.nswt.osp.ServiceMethodDefinition;
 ...
 * @version 1.0
 */
public class DefaultOspContextTest {
    ...
    public void testWithoutGroupService() {
        ...
        OspContext registryMethod = new DefaultOspContext(context);
    ...
    public void testWithGroupService() {
        ...
        OspContext ospContext = new DefaultOspContext(context);

        //method1:都在ServiceMethodGroup中定义，在ServiceMethod中直接采用
        ServiceMethodHandler handler = ospContext.getServiceMethodHandler("service.method1", "1.0");
        ServiceMethodDefinition definition = handler.getServiceMethodDefinition();
        assertNotNull(definition);
        assertEquals(definition.getMethod(), "service.method1");
        assertEquals(definition.getMethodTitle(), "测试方法1");
        ...
        ServiceMethodHandler handler2 = ospContext.getServiceMethodHandler("service.method2", "2.0");
    ...
    public void testIngoreSignField() {
        ...
        Class serviceClass = IgnoreSignOspRequestService.class;
        when(context.getType("method1")).thenReturn(serviceClass);

        OspContext ospContext = new DefaultOspContext(context);

        ServiceMethodHandler method1 = ospContext.getServiceMethodHandler("method1", "1.0");
    ...
    @ServiceMethodBean
    public class IgnoreSignOspRequestService {

        @ServiceMethod(method = "method1", version = "1.0")
        public Object method1(FooOspRequest request) {
            return null;
        }
    }

    public class FooOspRequest extends AbstractOspRequest {
