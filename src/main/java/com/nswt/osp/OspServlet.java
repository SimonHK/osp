 ...
 * <pre>
 *
 * HttpServlet是Osp框架的总入口，提供了多个定制Osp框架的配置参数：
 * 1.Osp会自己扫描Spring容器并加载之{@link SessionChecker}、{@link AppSecretManager}及{@link Interceptor}的Bean。也可以通过
 ...
 *
 *   如果既没有使用Servlet参数指定，也没有在Spring容器中配置，则Osp使用{@link DefaultSessionChecker}和{@link FileBaseAppSecretManager}
 * 作为{@link SessionChecker}和{@link AppSecretManager}的实现类。
 *
 * 2.可通过"errorResourceBaseName"指定错误资源文件的基名，默认为“i18n/osp/ropError”.
 *
 * </pre>
 *
 * @author Admin
 * @version 1.0
 */
public class OspServlet extends HttpServlet {
    ...
    public void init(ServletConfig servletConfig) throws ServletException {
        ...
        if (this.serviceRouter == null) {
            logger.error("在Spring容器中未找到" + ServiceRouter.class.getName() +
                    "的Bean,请在Spring配置文件中通过<aop:annotation-driven/>安装NSWT osp框架。");
        }
    }
