...
import com.nswt.osp.impl.DefaultServiceAccessController;
import com.nswt.osp.impl.SimpleOspRequestContext;
import com.nswt.osp.request.UploadFileUtils;
...
public class DefaultSecurityManager implements SecurityManager {
    ...
    public MainError validateSystemParameters(OspRequestContext context) {
        OspContext ospContext = context.getOspContext();
        ...
        if (context.getMethod() == null) {
            ...
            if (!ospContext.isValidMethod(context.getMethod())) {
        ...
        if (context.getVersion() == null) {
            ...
            if (!ospContext.isValidVersion(context.getMethod(), context.getVersion())) {
    ...
    public MainError validateOther(OspRequestContext rrctx) {
    ...
    private MainError checkUploadFile(OspRequestContext rrctx) {
    ...
    private MainError checkInvokeTimesLimit(OspRequestContext rrctx) {
     ...
     */
    private MainError validateHttpAction(OspRequestContext ospRequestContext) {
        MainError mainError = null;
        HttpAction[] httpActions = ospRequestContext.getServiceMethodDefinition().getHttpAction();
        if (httpActions.length > 0) {
            boolean isValid = false;
            for (HttpAction httpAction : httpActions) {
                if (httpAction == ospRequestContext.getHttpAction()) {
                    isValid = true;
                    break;
                }
            }
            if (!isValid) {
                mainError = MainErrors.getError(
                        MainErrorType.HTTP_ACTION_NOT_ALLOWED, ospRequestContext.getLocale(),
                        ospRequestContext.getMethod(), ospRequestContext.getVersion(),
                        ospRequestContext.getHttpAction());
            }
        }
        return mainError;
    }
    ...
    private MainError checkServiceAccessAllow(OspRequestContext smc) {
    ...
    private MainError validateBusinessParams(OspRequestContext context) {
        List<ObjectError> errorList =
                (List<ObjectError>) context.getAttribute(SimpleOspRequestContext.SPRING_VALIDATE_ERROR_ATTRNAME);

        //将Bean数据绑定时产生的错误转换为Osp的错误
     ...
     * @return
     */
    private MainError checkSign(OspRequestContext context) {
        ...
        if (context.isSignEnable()) {
            if (!context.getServiceMethodDefinition().isIgnoreSign()) {
                if (context.getSign() == null) {
                    ...
                    if (signSecret == null) {
                        throw new OspException("无法获取" + context.getAppKey() + "对应的密钥");
                    }

                    String signValue = OspUtils.sign(needSignParams, signSecret);
     ...
     * @return
     */
    private MainError checkSession(OspRequestContext context) {
        //需要进行session检查
    ...
    private boolean isValidSession(OspRequestContext smc) {
    ...
    private MainError toMainErrorOfSpringValidateErrors(
            List<ObjectError> allErrors, Locale locale,OspRequestContext context) {
    ...
    private MainError getBusinessParameterMainError(
            List<ObjectError> allErrors, Locale locale, SubErrorType subErrorType,OspRequestContext context) {
