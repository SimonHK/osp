...
public class AnnotationServletServiceRouterFactoryBean
        ...
        try {
            if (StringUtils.hasText(threadFerryClassName)) {
                ...
                if (!ClassUtils.isAssignable(ThreadFerry.class, threadFerryClass)) {
                    throw new OspException(threadFerryClassName + "没有实现"
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        //注册监听器
        ArrayList<OspEventListener> listeners = getListeners();
        if (listeners != null) {
            for (OspEventListener listener : listeners) {
    ...
    private ArrayList<OspEventListener> getListeners() {
        Map<String, OspEventListenerHodler> listenerMap = this.applicationContext.getBeansOfType(OspEventListenerHodler.class);
        if (listenerMap != null && listenerMap.size() > 0) {
            ArrayList<OspEventListener> ospEventListeners = new ArrayList<OspEventListener>(listenerMap.size());

            //从Spring容器中获取Interceptor
            for (OspEventListenerHodler listenerHolder : listenerMap.values()) {
                ospEventListeners.add(listenerHolder.getOspEventListener());
            }
