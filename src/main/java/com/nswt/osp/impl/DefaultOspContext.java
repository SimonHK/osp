 ...
 * @version 1.0
 */
public class DefaultOspContext implements OspContext {

    protected static Logger logger = LoggerFactory.getLogger(DefaultOspContext.class);
    ...
    public DefaultOspContext(ApplicationContext context) {
    ...
    /**
     * 扫描Spring容器中的Bean，查找有标注{@link ServiceMethod}注解的服务方法，将它们注册到{@link OspContext}中缓存起来。
     *
    ...
    private void registerFromContext(final ApplicationContext context) throws BeansException {
        if (logger.isDebugEnabled()) {
            logger.debug("对Spring上下文中的Bean进行扫描，查找OSP服务方法: " + context);
        }
                ...
                ReflectionUtils.doWithMethods(handlerType, new ReflectionUtils.MethodCallback() {
                            public void doWith(Method method) throws IllegalArgumentException, IllegalAccessException {
                                ...
                                if (method.getParameterTypes().length > 1) {//handler method's parameter
                                    throw new OspException(method.getDeclaringClass().getName() + "." + method.getName()
                                            + "的入参只能是" + OspRequest.class.getName() + "或无入参。");
                                } else if (method.getParameterTypes().length == 1) {
                                    Class<?> paramType = method.getParameterTypes()[0];
                                    if (!ClassUtils.isAssignable(OspRequest.class, paramType)) {
                                        throw new OspException(method.getDeclaringClass().getName() + "." + method.getName()
                                                + "的入参必须是" + OspRequest.class.getName());
                                    }
                                    boolean ospRequestImplType = !(paramType.isAssignableFrom(OspRequest.class) ||
                                            paramType.isAssignableFrom(AbstractOspRequest.class));
                                    serviceMethodHandler.setOspRequestImplType(ospRequestImplType);
                                    serviceMethodHandler.setRequestType((Class<? extends OspRequest>) paramType);
                                } else {
    ...
    public static List<String> getIgnoreSignFieldNames(Class<? extends OspRequest> requestType) {
    ...
    private List<String> getFileItemFieldNames(Class<? extends OspRequest> requestType) {
